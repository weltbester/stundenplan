...
try:
    ...
    rc.COMMAND_GROUPS = {
        "main.py": [
            ...
            {
                "name": "Export & Anzeige",
                "commands": ["export", "show", "browse", "run"],
            },
            {
                "name": "Analyse",
...
def cmd_template(output: str):
    ...
    console.print(
        ...
        "  [cyan]Fächer[/cyan]        – Fächerliste (bitte anpassen)\n"
        "  [cyan]Stundentafel[/cyan]  – Fach × Jahrgang Matrix (bitte anpassen)\n"
...
def cmd_import(datei: Path, save_json: bool, json_path: str,
    ...
    from data.excel_import import (
        import_from_excel, import_from_csv, ExcelImportError
    )

    console.print(f"[bold]Importiere:[/bold] {datei}")
    try:
        if datei.is_dir() or datei.suffix.lower() == ".csv":
            school_data, report = import_from_csv(datei, config)
        elif datei.suffix.lower() == ".xml":
            from data.untis_import import import_from_untis
            from models.school_data import FeasibilityReport
            school_data, untis_report = import_from_untis(datei, config)
            untis_report.print_rich()
            report = FeasibilityReport(
                is_feasible=len(untis_report.errors) == 0,
    ...
    if save_json:
        out_path = Path(json_path)
        if save_versioned:
            actual_path = school_data.save_versioned(out_path)
            console.print(f"[green]✓[/green] Daten gespeichert: {actual_path}")
        else:
...
@click.option("--incremental", is_flag=True, default=False,
              ...
              "Lösung.")
@click.option("--prev-solution", default=None,
              help="Pfad zur vorherigen Lösung für --incremental (Standard: --output Pfad).")
@click.option("--save-versioned", is_flag=True, default=False,
              help="Lösung mit Zeitstempel im Dateinamen speichern.")
def cmd_solve(time_limit, small, json_path, output, pins_path, diagnose, verbose,
              no_soft, weights, two_pass, incremental, prev_solution, save_versioned):
    ...
    """
    from models.school_data import SchoolData
    from solver.scheduler import ScheduleSolver, RoomAssignmentError
    from solver.pinning import PinManager
    ...
    if incremental:
        from solver.scheduler import ScheduleSolution as _SS, ScheduleSolver as _SV
        prev_path = Path(prev_solution) if prev_solution else Path(output)
        if prev_path.exists():
            old_sol = _SS.load_json(prev_path)
            affected = _SV._identify_affected_teachers(old_sol, data)
            console.print(
                f"[cyan]Inkrementell: {len(affected)} betroffene Lehrer, "
                f"{len(data.teachers) - len(affected)} fixiert.[/cyan]"
            )
            from solver.pinning import PinnedLesson
            for entry in old_sol.entries:
                if entry.teacher_id not in affected and not entry.is_coupling:
                    pin_manager.add_pin(PinnedLesson(
            ...
            console.print(f"[cyan]{len(pin_manager)} Stunden gepinnt.[/cyan]")
        else:
            console.print(
                f"[yellow]--incremental: Keine vorherige Lösung unter {prev_path} "
                "— vollständiger Solve.[/yellow]"
            )
    ...
    try:
        ...
        console.print(f"[red bold]Fachraum-Engpass:[/red bold] {e}")
        sys.exit(1)
    ...
    if solution.solver_status == "FEASIBLE":
        console.print(
            "[yellow]Stundenplan gefunden (nicht optimal — Zeitlimit erreicht).[/yellow]\n"
            f"Tipp: Erhöhen Sie das Zeitlimit mit "
            f"[bold]--time-limit {data.config.solver.time_limit_seconds * 2}[/bold] "
            "für eine bessere Lösung."
        )

    if solution.solver_status not in ("OPTIMAL", "FEASIBLE"):
        if solution.solver_status == "UNKNOWN":
            console.print(
                f"[yellow]Kein Stundenplan innerhalb von "
                f"{solution.solve_time_seconds:.0f}s gefunden.[/yellow]\n"
                f"Tipp: Erhöhen Sie das Zeitlimit mit "
                f"[bold]--time-limit {data.config.solver.time_limit_seconds * 2}[/bold] "
                "oder verwenden Sie [bold]--small[/bold] zur Diagnose mit Mini-Datensatz."
            )
        if diagnose:
    ...
    out_path = Path(output)
    if save_versioned:
        actual_path = solution.save_versioned(out_path)
        console.print(f"[green]✓[/green] Lösung gespeichert: {actual_path}")
    else:
...
@click.option("--solution-path", default=str(DEFAULT_SOLUTION_JSON),
              show_default=True, help="Pfad zur Lösungs-JSON.")
def cmd_browse(json_path: str, solution_path: str):
    """[bold]Interaktiver TUI-Browser[/bold] für den Stundenplan.

    Navigation: [cyan]j/k[/cyan] oder [cyan]↑↓[/cyan] in der Liste,
    [cyan]/[/cyan] für Suche, [cyan]q[/cyan] oder [cyan]Escape[/cyan] zum Beenden.
    """
    from models.school_data import SchoolData
    from solver.scheduler import ScheduleSolution
    from export.tui_browser import StundenplanApp
    ...
    try:
        StundenplanApp(solution, school_data).run()
    except ImportError as e:
        console.print(f"[red bold]TUI nicht verfügbar:[/red bold] {e}")
        console.print(
            "Installieren Sie textual mit: [bold]pip install textual>=0.60[/bold]"
        )
        sys.exit(1)
