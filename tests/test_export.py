...
class TestTuiRenderer:
    """TUI-Renderer erzeugt korrekte Dimensionen."""

    def test_render_class_rows_dimensions(self, tui_solution):
        solution, data = tui_solution
        from export.tui_renderer import render_class_rows
        rows = render_class_rows("5a", solution, data, solution.config_snapshot)
        assert len(rows) > 0
        for row in rows:
            assert len(row) == 7, (
                f"Zeile hat {len(row)} Spalten, erwartet 7: {row}"
            )

    def test_render_teacher_rows_dimensions(self, tui_solution):
        solution, data = tui_solution
        from export.tui_renderer import render_teacher_rows
        t_id = solution.entries[0].teacher_id if solution.entries else "T01"
        rows = render_teacher_rows(t_id, solution, data, solution.config_snapshot)
        assert len(rows) > 0
        for row in rows:
            assert len(row) == 7, f"Zeile hat {len(row)} Spalten: {row}"

    def test_render_teacher_rows_gap_label(self, tui_solution):
        """Springstunden werden korrekt markiert."""
        solution, data = tui_solution
        from export.tui_renderer import render_teacher_rows
        for teacher in data.teachers:
            rows = render_teacher_rows(
                teacher.id, solution, data, solution.config_snapshot
            )
            for row in rows:
                for cell in row[2:]:
